...
5. Replace the example resource.
   ...
   If your project never needs SQL persistence, you can delete `internal/adapter/postgres`, `sqlc.yaml`, the `sqlc`/`sqlc-check`/`migrate`/`test-integration` Moon tasks, the `migrate` subcommand, the `--store`/`--database-url`/`--db-max-conns` config flags, the `.moon/proto/{sqlc,goose}.toml` plugins and their `.prototools` pins, the `run.build-tags` entry in `.golangci.yml`, and the pgx/goose/testcontainers Go dependencies, then run `go mod tidy` (sqlc and the goose CLI are Proto plugins, not Go modules). If your project never uses the in-memory store, drop `internal/adapter/memory` and default `--store` to `postgres` instead.
